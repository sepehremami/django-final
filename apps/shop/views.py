...
class ProductDetailView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        pricing_data = Pricing.objects.filter(subproduct=subproduct).order_by(
            "create_date"
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            cart, created = OrderInfo.objects.get_or_create(
                user=self.request.user, order_status=2
            )
            order_item, created = OrderItem.objects.get_or_create(
                product_id=subid, order=cart
            )
        except Exception as e:
            print(e)
